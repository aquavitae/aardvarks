...
import { log, flankingFlag, moduleName } from './common.js';
...
const countAdjacent = (target) => {
  const tokenStates = canvas.tokens.ownedTokens.map((tok) => mapState(tok, target));
...
const checkFlanking = async (target) => {
  ...
  if (n >= 2) {
    await game.user.setFlag(moduleName, flankingFlag, n);
  } else {
    await game.user.unsetFlag(moduleName, flankingFlag);
  }
};

const onTargetToken = async (user, target, state) => {
  if (state) {
...
function getAttackToHit(wrapped) {
  ...
  const mwak = this?.data?.data?.actionType === 'mwak';
...
export default async () => {
  Hooks.on('targetToken', async (user, target, state) => {
    await onTargetToken(user, target, state);
  });
